...
public class CheckStateListener implements ICheckStateListener, SelectionListener {
     ...
     */
    private void setMergedResourcesChecked() {
        CheckboxTreeViewer resourcesTree = page.getResourcesTree();
        for (IFile file : javaGeneratorWrapper.getMergeableFiles()) {
            IJavaElement jElem = JavaCore.create(file);
            if (jElem != null) {
                resourcesTree.setChecked(jElem, true);
            } else {
                resourcesTree.setChecked(file, true);
            }
        }
    }
    ...
    private void setAllResourcesChecked() {
        CheckboxTreeViewer resourcesTree = page.getResourcesTree();
        // TODO improve, getAllFiles usually gets more files than needed
        for (IFile file : javaGeneratorWrapper.getAllFiles()) {
            IJavaElement jElem = JavaCore.create(file);
            if (jElem != null) {
                resourcesTree.setChecked(jElem, true);
            } else {
