...
public class AnalyzeInputJob extends AbstractCobiGenJob {
   ...
   *
   * @param cobigen CobiGen instance
   * @param inputs input objects to be analyzed
   */
  ...
  public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
    ...
    try {
      ...
      SubMonitor subMonitor = SubMonitor.convert(monitor, this.inputs.size() + 1);
      subMonitor.beginTask("Determine matching templates...", this.inputs.size() + 1);

      for (Object input : this.inputs) {
        subMonitor.split(1);
        this.resultMatchingTemplates.addAll(this.cobigen.getMatchingTemplates(input));
      }
      LOG.info("Determine if input is container...");
      this.resultSingleNonContainerInput = this.inputs.size() == 1
          && !this.cobigen.combinesMultipleInputs(this.inputs.get(0));
      monitor.done();
  ...
  public List<TemplateTo> getResultMatchingTemplates() {
    ...
    return this.resultMatchingTemplates;
  }
  ...
  public boolean isResultSingleNonContainerInput() {
    ...
    return this.resultSingleNonContainerInput;
  }
