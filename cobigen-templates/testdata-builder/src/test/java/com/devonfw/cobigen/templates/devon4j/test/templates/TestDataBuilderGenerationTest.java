 ...
 */
public class TestDataBuilderGenerationTest extends AbstractMavenTest {
  ...
  public static void setupDevTemplates() throws URISyntaxException, IOException {
    ...
    templatesProject = new File(
        TestDataBuilderGenerationTest.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile()
            .getParentFile().toPath();

    Path utilsPom = new File(
        TestDataBuilderGenerationTest.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile()
            .getParentFile().toPath().resolve("src/test/resources/utils/pom.xml");
