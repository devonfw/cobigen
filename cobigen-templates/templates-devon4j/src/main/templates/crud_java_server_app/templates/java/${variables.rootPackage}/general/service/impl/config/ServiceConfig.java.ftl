...
public class ServiceConfig extends WsConfigurerAdapter {
  ...
  @Value("${r"${security.expose.error.details}"}")
  ...
  @Bean
  public EndpointImpl jaxWsServices() {
  
  String[] serviceList = this.applicationContext.getBeanNamesForAnnotation(WebService.class);
  EndpointImpl endpoint = null;
  for (String serviceName : serviceList) {
  Class<?> className = this.applicationContext.getType(serviceName);
  try {
  endpoint = new EndpointImpl(springBus(), Class.forName(className.getTypeName()).getConstructor().newInstance());
