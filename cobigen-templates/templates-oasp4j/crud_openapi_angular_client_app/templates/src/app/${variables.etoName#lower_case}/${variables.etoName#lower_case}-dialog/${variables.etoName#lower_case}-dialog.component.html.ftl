...
<form (ngSubmit)="dialogRef.close(items)" #dialogForm="ngForm">
  ...
  <mat-input-container>
    <input matInput type="${getType(OaspUtil.getOaspTypeFromOpenAPI(property, false, false))}" name = "${property.name}" [placeholder]= "'${variables.component}.${variables.etoName}.columns.${property.name}' | translate" [(ngModel)] = "items.${property.name}" required>
  </mat-input-container>
  </#list>
  <mat-dialog-actions>
