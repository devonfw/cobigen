          ...
          <div layout="row" class="pad-left-md pad-right-md" style="align-items:center; border-bottom: 1px solid lightgrey" flex>
            ...
            <#list model.properties as property>
              <mat-form-field color="accent">
                <input matInput placeholder="${property.name?cap_first}" [(ngModel)]="searchTerms.${property.name}" name="${property.name}">
              </mat-form-field>
            </#list>
            </div>
            ...
            <#list model.properties as property>
              <mat-form-field color="accent" class="pad-sm" flex>
                <input matInput type="${JavaUtil.getAngularType(property.type)}" placeholder="${property.name?cap_first}" [(ngModel)]="searchTerms.${property.name}" name="${property.name}">
              </mat-form-field>
            </#list>
            </div>
          </div>
