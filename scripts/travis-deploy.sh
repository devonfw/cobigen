...
if [ "$TRAVIS_BRANCH" != "master" ] && [ "$TRAVIS_BRANCH" != "gh-pages" ] && [ "$TRAVIS_BRANCH" != "dev_eclipseplugin" ] && [ "$TRAVIS_BRANCH" != "dev_oomph_setup" ]; then
  ...
  if [ "$TRAVIS_BRANCH" != "dev_core" ] && [ "$TRAVIS_BRANCH" != "dev_mavenplugin" ]; then
    ...
    mvn package -U bundle:bundle -Pp2-bundle,p2-build-photon,p2-build-ci -Dmaven.test.skip=true
    mvn install -U bundle:bundle -Pp2-bundle,p2-build-photon,p2-build-ci p2:site -Dmaven.test.skip=true
    mvn deploy -U -Pp2-build-photon,p2-build-ci -Dmaven.test.skip=true -Dbintray.repository=cobigen.p2.ci
  ...
  mvn deploy -U -Dmaven.test.skip=true -Pp2-build-photon,p2-build-ci -Dbintray.repository=cobigen.p2.ci
fi
