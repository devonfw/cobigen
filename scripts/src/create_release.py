...
def make_temp_directory():
    temp_dir = tempfile.mkdtemp(prefix="cobigen_")
    try:
        yield temp_dir
    finally:
        count = 1
        while True:
            log_info("Try deleting temporary files (" + str(count) + ")")
            try:
                shutil.rmtree(r'\\?\ '.strip() + temp_dir)
                break
            except OSError as e:
                if count > 5:
                    raise e
                ...
                if access_error:
                    log_info("Wait for windows file locks and deleting temporary files...")
                    time.sleep(10)

@contextlib.contextmanager
def create_git_repo(config: Config):
...
with make_temp_directory() as temp_dir:
    ...
    config = Config()
    # default temp folder if no parameter is set. Need to set this here as the temp folder is deleted otherwise
    config.temp_root_path = temp_dir
    ...
    with create_git_repo(config) as git_repo:
        ...
        for file_name in list_of_changed_files:
            ...
            if not file_name.startswith(config.build_folder) and file_name != 'documentation\master-cobigen.asciidoc':
        ...
        def __deploy_m2_only(oss: bool, execpath: str = config.build_folder_abs):
            ...
            if oss:
                activation_str = "-Poss -Dgpg.keyname="+config.gpg_keyname + " -Dgpg.executable="+config.gpg_executable
            run_maven_process_and_handle_error("mvn clean -DskipTests=true deploy -U "+activation_str, execpath=execpath)
        ...
        if config.dry_run or config.test_run:
            ...
            if config.branch_to_be_released not in [config.branch_eclipseplugin, config.branch_mavenplugin, config.branch_core, config.branch_javaplugin, config.branch_openapiplugin, 'dev_cli']:
                ...
                __deploy_m2_only(config.oss, os.path.join(config.build_folder_abs, "cobigen-javaplugin"))
                ...
                __deploy_m2_only(config.oss, os.path.join(config.build_folder_abs, "cobigen-openapiplugin"))
                __deploy_m2_as_p2(config.oss, os.path.join(config.build_folder_abs, "cobigen-openapiplugin"))
            elif config.branch_to_be_released == config.branch_mavenplugin:
                # need to run it twice to as of the deployment is skipped for all modules if systemtest module is running at the end
                __deploy_m2_only(config.oss, os.path.join(config.build_folder_abs, "cobigen-maven-plugin"))
                __deploy_m2_only(config.oss, os.path.join(config.build_folder_abs, "cobigen-maven-test"))
