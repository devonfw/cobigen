...
class GitRepo:
    ...
    def merge(self, source: str, target: str) -> None:
        if self.__config.dry_run:
            print_info_dry("Would merge from "+source+" to " + target)
            return
    ...
    def update_submodule(self, submodule_path: str) -> None:
        ...
        print_info("Changing the "+self.__config.wiki_version_overview_page + " file, updating the version number...")
