package com.capgemini.cobigen.sample.common;
...
public class AbstractLayerImpl {
    ...
    protected <C, P> P mapInitialToTargetLayerEntity(C entity, Class<P> outputEntityType) {
        ...
        if (entity == null) {
            this.LOG.debug("No entity to map.");
            return null;
        }

        this.LOG.debug("Mapping: '" + entity.getClass().getName() + "' --> '" + outputEntityType.getName()
            + "'.");

        return (this.dozer.map(entity, outputEntityType));
    }
    ...
    protected <C, P> List<P> mapInitialToTargetLayerEntity(List<C> entities, Class<P> outputEntityType) {
        if (entities == null || entities.isEmpty()) {
            this.LOG.debug("No entities to map.");
            return null;
        }
    ...
    protected <P, S, C> C mapSuperTypeToOtherLayersSubType(S initialLayerSuperClassEntity,
        ...
        this.LOG.debug("Try to cast superEntity '" + initialLayerSuperClassEntity.getClass().getName()
        ...
        if (initialLayerSubClassType.isInstance(initialLayerSuperClassEntity)) {
            ...
            this.LOG.debug("Sucessfully casted '" + initialLayerSuperClassEntity.getClass().getName()
                + "' to '" + initialLayerSubClassType.getName() + "'.");
        ...
        this.LOG.debug("There is no relationship between the super entity '"
