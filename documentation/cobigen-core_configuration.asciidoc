...
The CobiGen configuration file located at: `$home/.cobigen` can be used to set the path to the `templates` as well as the `template sets` project.

=== Templates

For a templates folder simply set the `templates` parameter like this:

```
templates=C:\\project\\ide\\conf\\templates
```

For a templates jar simply set the `templates` parameter like this:
...
For a template-sets folder simply set the `template-sets` parameter like this:

```
template-sets=C:\\project\\ide\\conf\\template-sets
```

For a template-sets jar simply set the `template-sets` parameter like this:
