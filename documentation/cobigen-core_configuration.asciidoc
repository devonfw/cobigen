   ...
   |- template-set1
     |- src
       |- main
         |- templates
```
Whereas`template-set1` is the name of the template you want to upgrade.
[start=3]
3. Navigate to `CobiGen_Templates\src\main\java` and copy the `java` folder into your newly created `main` folder.
...
```
[start=4]
4. Navigate to `CobiGen_Templates\src\main\templates\template-set1` and copy all of it's contents
into your newly generated templates folder (`template-sets\adapted\templateSet1\src\main\templates`).

6. Switch to `template-sets\adapted\templateSet1\src\main\templates` and create a new 'context.xml' file, with the following (to your needs adapted) content:
```
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<contextConfiguration xmlns="http://capgemini.com/devonfw/cobigen/ContextConfiguration" version="3.0">
    <trigger id="templateSet1" type="java" templateFolder="templateSet1" inputCharset="UTF-8">
        <containerMatcher type="package" value="someValue" retrieveObjectsRecursively="false"/>
        <matcher type="fqn" value="*" accumulationType="NOT">
            <variableAssignment type="constant" key="foo" value="BLUBB1"/>
        </matcher>
    </trigger>
    <tags>
        <tag name="PLACEHOLDER---This tag was inserted through the upgrade process and has to be changed manually---PLACEHOLDER"/>
    </tags>
    <links>
        <link url="PLACEHOLDER---This tag was inserted through the upgrade process and has to be changed manually---PLACEHOLDER"/>
    </links>
</contextConfiguration>
```
[start=7]
7. Switch to `template-sets\adapted\templateSet1` and create a new `pom.xml`. With the following (to your needs adapted) content:
