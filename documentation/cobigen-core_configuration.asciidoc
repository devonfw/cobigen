...
```
[start=4]
4. Navigate to the template you want to upgrade e.g. `CobiGen_Templates\src\main\resources\template-set1` and copy all of it's contents
into your newly generated templates folder (`template-sets\adapted\templateSet1\src\main\resources`).

6. Switch to `template-sets\adapted\templateSet1\src\main\resources` and create a new 'template-set.xml' file (which combines the context.xml and templates.xml), with the following content (adapted to your needs):
```
<?xml version="1.0" encoding="UTF-8"?>
...
your template from the old 'context.xml' (located here: `CobiGen_Templates\src\main\resources`) and 'templates.xml' file (located here: `CobiGen_Templates\src\main\resources\template1`) (e.g. crud-java-server-app).
[start=7]
