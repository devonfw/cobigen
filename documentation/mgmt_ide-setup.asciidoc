...
You can also manually install the PDE and sources by opening Eclipse, switching to *Help/Install new Software*, choosing *Eclipse Project latest release repository - http://download.eclipse.org/eclipse/updates/latest* from the dropdown menu next to *Add* and ticking *Eclipse Plugin Development Tools*.
