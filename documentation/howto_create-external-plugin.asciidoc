...
On the new version, we have implemented a handler (`ExternalProcessHandler`) which connects through TCP/IP connection to a server (normally on localhost:5000). This server can be implemented in any language (.Net, Node.js, Python...) it just needs to implement an API contract defined here link:https://github.com/jdiazgon/cobigen-template-plugin/blob/master/APIContract.yml[here]. The most important services are the input reading and merging:

image::images/howtos/todo-plugin/newCobiGen.png[Old CobiGen,width="450"link="images/howtos/todo-plugin/newCobiGen.png"]
...
* Creation of the server: The server can be programmed in any language that is able to implement REST services endpoints. The API that it will have to implement is defined with link:https://github.com/jdiazgon/cobigen-template-plugin/blob/master/APIContract.yml[this contract].
