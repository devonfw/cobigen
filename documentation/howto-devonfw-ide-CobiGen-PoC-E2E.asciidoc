...
== Prerequisites
Download and install devonfw IDE https://devonfw.com/website/pages/docs/devonfw-guide_ide.wiki_setup.asciidoc.html[here],
...
For this it is necessary to open a CMD and open the folder where devonfw is installed and launch the command below under the following path: `_workspaces/main_`. +
...
`C:\projects\my-app\workspaces\main`
...
*BEFORE creating an Entity class, remember to create the tables, because this POC is using an H2 Database!*
...
*_WARNING 1_*: Please note in this folder there is only one file (0001). You have to add the other files in progressive number order (i.e. 0002)! +
*_WARNING 2_*: Please note that there are 2 underscores in the name!

image:images/howtos/e2e_gen/image64.png[SQL file]

[arabic]
Now create another SQL file (`i.e: V0003__PopulateTables-myapp.sql`) and add following script about the INSERT to populate the table created before.
The script must be inserted at the same path used before: _`/myapp-core/src/main/resources/db/migration/1.0`_

*_WARNING 1_*: Please note in this folder there is only one file (0001). You have to add the other files in progressive number order (i.e. 0003)! +
*_WARNING 2_*: Please note that there are 2 underscores in the name!
...
[arabic]
Create a package ```*com.example.domain.myapp.employeemanagement.dataaccess.api*``` under the folder _`myapp-core/src/main/java`_. +
...
Then generate `getters` and `setters` for all attributes, as you can see in the image below:

image:images/howtos/e2e_gen/image67.png[`getter & setter`]
image:images/howtos/e2e_gen/image68.png[`getter & setter`]

[arabic]
Use CobiGen to generate code. First, right-click on `EmployeeEntity` -> `CobiGen` and select `Update Template`. Next, right-click on `EmployeeEntity` -> CobiGen -> Generate and tick the boxes as can be seen in the picture:
...
Next, add some properties under your _application.properties_ file, in the `myapp-core` folder in the _resources/config_:
...
*BEFORE generating the `FE`*, please start the Tomcat server to check that `BE` Layer has been generated properly.

To start the server, right click on _`SpringBootApp.java`_ -> _run as -> Java Application_
...
Then, you have to create a `POST` Request for the LOGIN and insert the JSON containing the username and password _admin_ in the body:

image:images/howtos/e2e_gen/image72.png[postman]

*_WARNING_*: please note that the body of the request must be JSON type!

If the request succeeded, i.e. *Status: 200 OK*, we can create a NEW `GET` Request to get one employee.

Create a new `GET` type request in Postman and insert the URL specified before:
[source, URL]
----
http://localhost:8081/services/rest/employeemanagement/v1/employee/1
----
...
Check that the response returned *Status: 200 OK* and that you can see the following employee.

image:images/howtos/e2e_gen/image73.png[postman]

After successfully testing the `BE`, is time to go to create the `FE` !
...
[arabic]
IMPORTANT if you download the zip of the source code of your `FE` application, the name of the app MUST BE *devon4ng-application-template* and you can extract it into your devonfw folder, under `_workspaces/main_`.

After downloading the `APP`, you can open the application with your favorite IDE (IntelliJ, Visual Studio Code, ...) +
