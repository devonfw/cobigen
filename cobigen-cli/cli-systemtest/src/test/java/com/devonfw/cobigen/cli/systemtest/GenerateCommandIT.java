...
public class GenerateCommandIT extends AbstractCliTest {
    ...
    /**
     * Integration test of the generation of templates from a Java Entity. It does
     * not specify the project to generate the
     * folders to.
     *
    ...
    public void generateFromEntityTest() throws Exception {
        ...
        assertThat(baseProject.toPath()
                .resolve("src/main/java/com/maven/project/sampledatamanagement/dataaccess/api/repo"))
                .exists();
    }

    /**
     *
     * Test the upgrade process from templates in a given path and generates from
     * the new template-sets structure
     *
     * @throws Exception test fails
     */
    @Test
    ...
    /**
     * Integration test of the generation from a templates jar using a utility class
     * with an extra dependency. See:
    ...
    /**
     * Integration test of the generation of templates from a Java Entity. It will
     * generate all the templates in the
    ...
    public void generateFromEntityWithOutputRootPathTest() throws Exception {
        ...
        assertThat(
                outputRootPath.toPath()
                        .resolve("src/main/java/com/maven/project/sampledatamanagement/dataaccess/api/repo"))
                .exists();
    }

    /**
     * Integration test of the generation of templates from an OpenAPI file. It will
     * generate all the templates in the
     * output root path passed.
     *
     * @throws Exception test fails
    ...
    /**
     * Integration test of the generation of templates from a Java Entity with
     * number selection.
     *
     * @throws Exception test fails
     */
    ...
    /**
     * Integration test of the generation of templates from an input file whose path
     * contains spaces and quotes.
     *
     * @throws Exception test fails
     */
    @Test
    ...
    /**
     * Integration test of the generation and validation of the class path cache.
     * The test will execute a generation twice
     * this requires some cleanup.A deleted dependencies will be detected by the
     * validating process in MavenUtil and will
            ...
            if (f.getName().startsWith("pom-cp-")) {
                ...
                for (int i = 0; i >= 0; i--) { // reverse
                    if (dependenciesFromCacheArray[i].contains("javax.persistence")) {
                        FileUtils.delete(new File(dependenciesFromCacheArray[i])); // delete a randomly picked
                                                                                   // dependency
                    }
                }
            }
