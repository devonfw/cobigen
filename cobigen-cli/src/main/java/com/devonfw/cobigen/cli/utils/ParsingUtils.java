...
public class ParsingUtils {
    ...
    public static void formatJavaSources(Set<Path> generatedFiles) throws FormatterException {
        Set<Path> format = generatedFiles;
        ...
        while (itr.hasNext()) {
            try {
                ...
                String formatedCode = formatter.formatSource(unformatterContent);
                FileOutputStream File = new FileOutputStream(inputStream.toString());
                File.write(formatedCode.getBytes());
                File.close();
            } catch (IOException e) {
