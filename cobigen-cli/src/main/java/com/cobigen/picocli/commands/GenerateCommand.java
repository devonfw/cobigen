...
public class GenerateCommand implements Callable<Integer> {
	 ...
	 * 
	 * @param inputFile input file which we are going to parse to find out whether
	 *                  it is a valid file
	...
	private Boolean parseRelativePath(File inputFile, int index) {
		try {
			...
			logger.error(
					"Your <inputFile> " + inputFile.getAbsolutePath() + " does not exist, please use a valid file.");
		}
		return false;
	}

	/**
	 * Checks whether the current output root path is valid. It can be either null
	 * because it is an optional parameter or either a folder that exists.
	 * 
	 ...
	 * This method is trying to find the first pom.xml file and then getting the
	 * folder where is located
	 * 
		...
		try {
			...
			if (increments==null  ||  increments.size() < 1) {
				...
				for (int j = 0; j < increments.size(); j++) {
					userIncrements.add(j, matchingIncrements.get(increments.get(j)));
					logger.info("Increments are going to be generated " + userIncrements);
				}
	 ...
	 * project that is able to load the input file. We need this in order to perform
	 * reflection on the templates.
	 * 
	 * @param inputFile    input file the user wants to generate code from
	 * @param inputProject input project where the input file is located. We need
	 *                     this in order to build the classpath of the input file
