...
public class AngularMerger implements Merger {
    ...
    for (Element element : patchElements) {
      ...
      switch (element.tagName()) {
        case Constants.MD_NAV_LIST:
          AppComponent(fileDocBase, element, this.patchOverrides);
          break;
        case Constants.FORM:
          if (element.id().isEmpty()) {
            dataGrid(fileDocBase, docPatch, this.patchOverrides);
          } else {
            addDialog(fileDocBase, docPatch, this.patchOverrides);
          }
          break;
      }
   ...
   *
   * @param fileDocBase the existent app.component.html
   * @param element the nav-list {@link Element}
   * @param patchOverrides if <code>true</code>, conflicts will be resolved by using the patch contents<br>
   *        if <code>false</code>, conflicts will be resolved by using the base contents
   */
   ...
   *
   * @param fileDocBase the existent dataGrid.component.html
   * @param docPatch the {@link Document} that patches
   * @param patchOverrides if <code>true</code>, conflicts will be resolved by using the patch contents<br>
   *        if <code>false</code>, conflicts will be resolved by using the base contents
   */
   ...
   *
   * @param inputs the elements to extract the the values
   ...
   *
   * @param elements the group of {@link Elements}
  ...
  /**
   * The *ngIf attribute is parsed as lower case, causing errors on the compilation of the Angular2 client. This method
   * fixes this attribute
   *
   * @param fileDocBase the existent {@link Document}
   */
