...
public class VelocityTemplateEngine implements TextTemplateEngine {
  ...
  public VelocityTemplateEngine() {
    ...
    this.engine.setProperty(RuntimeConstants.RUNTIME_LOG_LOGSYSTEM,
        new LogChuteDelegate(LoggerFactory.getLogger(VelocityEngine.class)));
    this.engine.setProperty(RuntimeConstants.ENCODING_DEFAULT, "UTF-8");
    this.engine.setProperty(RuntimeConstants.FILE_RESOURCE_LOADER_CACHE, new Boolean(false));
    this.engine.setProperty(RuntimeConstants.RESOURCE_MANAGER_CLASS, ResourceManagerDelegate.class.getName());
    this.engine.setProperty(RuntimeConstants.RESOURCE_MANAGER_LOGWHENFOUND, new Boolean(true));
    this.engine.setProperty(RuntimeConstants.RESOURCE_MANAGER_CACHE_CLASS, NullResourceCache.class.getName());
  }
  ...
  public void process(TextTemplate template, Map<String, Object> model, Writer out, String outputEncoding) {
    ...
    try {
      vmTemplate = executeInThisClassloader(template.getRelativeTemplatePath(),
          (path) -> this.engine.getTemplate(path));
    } catch (Throwable e) {
  ...
  public void setTemplateFolder(Path templateFolderPath) {
    ...
    this.engine.setProperty(RuntimeConstants.FILE_RESOURCE_LOADER_PATH, templateFolderPath.toString());
  }

  /**
   * Execute a function within the classloader loading THIS class to circumvent from velocity classpath conflicts in
   * osgi environments
   *
   * @param <T> parameter type of the function
   * @param <R> return type of the function
   * @param param function parameter
   * @param exec function to be called
