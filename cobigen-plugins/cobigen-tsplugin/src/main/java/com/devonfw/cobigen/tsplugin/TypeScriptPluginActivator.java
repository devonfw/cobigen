...
public class TypeScriptPluginActivator implements GeneratorPluginActivator {
    ...
    private ExternalProcess externalProcess =
        new ExternalProcess(EXTERNAL_SERVER_DOWNLOAD_URL, MavenMetadata.ARTIFACT_ID, MavenMetadata.SERVER_VERSION,
            "/processmanagement/" + MavenMetadata.ARTIFACT_ID + "/");
    ...
    public List<Merger> bindMerger() {
        ...
        merger.add(new TypeScriptMerger(externalProcess, TSMERGE, false));
        merger.add(new TypeScriptMerger(externalProcess, TSMERGE_OVERRIDE, true));
        return merger;
    }

    @Override
    public List<TriggerInterpreter> bindTriggerInterpreter() {
        return Lists.<TriggerInterpreter> newArrayList(new TypeScriptTriggerInterpreter(externalProcess, TRIGGER_TYPE));
    }
