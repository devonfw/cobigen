...
public class TextAppender implements Merger {
  ...
  /**
   * Creates a new text appender which appends the patch to the base file. If {@link #withNewLineBeforehand} is set, the
   * patch will be appended by first adding a new line to the base file
   *
   * @param type of the text appender instance
  ...
  public TextAppender(String type, boolean withNewLineBeforehand) {
    ...
    switch (type) {
      case "textmerge_append":
        this.defaultStrat = MergeStrategy.APPEND;
        break;
      case "textmerge_appendWithNewLine":
        this.defaultStrat = MergeStrategy.APPEND;
        break;
      case "textmerge_override":
        this.defaultStrat = MergeStrategy.OVERRIDE;
        break;
    }
    this.type = type;
  ...
  public String merge(File base, String patch, String targetCharset) throws MergeException {
    ...
    try {
      if (lineDelimiterBase == null) {
        ...
        mergedString = merge(mergedString, StringUtil.consolidateLineEndings(patch, lineDelimiterBase),
            lineDelimiterBase);
      }
    } catch (Exception e) {
   ...
   *
   * @param base target {@link String} to be merged into
   ...
   * @throws Exception When there is some problem about anchors
   */
  public String merge(String base, String patch, String lineDelimiter) throws Exception {

    String mergedString = "";
    if (MergeUtil.hasAnchors(patch)) {
      LinkedHashMap<Anchor, String> splitBase = MergeUtil.splitByAnchors(base, this.defaultStrat, lineDelimiter);
      LinkedHashMap<Anchor, String> splitPatch = MergeUtil.splitByAnchors(patch, this.defaultStrat, lineDelimiter);
                ...
                if (tmpAnchor.getNewlineName().matches("(newline_).+")) {
                  switch (tmpAnchor.getNewlineName().toLowerCase()) {
                    case "newline_appendbefore":
                      toAppend += MergeUtil.appendText(toAppend, docPart, splitBase, false, false, lineDelimiter);
                      toAppend = MergeUtil.appendText(toAppend, docPart, splitPatch, true, true, lineDelimiter);
                  ...
                  switch (mergeStrat) {
                    ...
                    case OVERRIDE:
                      ...
                      break;
                    case NOMERGE:
                      toAppend = MergeUtil.appendText(toAppend, docPart, splitBase, false, true, lineDelimiter);
                      mergedString = MergeUtil.addTextAndDeleteCurrentAnchor(mergedString, toAppend, splitPatch,
                          splitBase, tmpAnchor);
                      break;
                    case ERROR:
                    ...
                    default:
                      throw new Exception("Implementation error, please create a new issue at "
                          ...
                          + "and provide your document and the faulty anchor " + tmpAnchor.getAnchor());
                  }
                }
      ...
      if (StringUtils.isNotEmpty(patch)) {
        if (this.type.equalsIgnoreCase("textmerge_override")) {
          mergedString = patch;
          return mergedString;
        } else if (this.withNewLineBeforehand) {
