...
public class AnchorTest {
     ...
     * anchor:something:anchorend) is converted to an empty String or the mergestrategy error(which will be an
     * empty string when getting the whole anchor), which will not match the regular expression for a correct
     * anchor that requires text between colons, thus throwing an exception when splitting a text by anchors.
     */
    @Test
    ...
    /**
     * Tests if getting the newline name puts _newline or newline_ correctly only if there is supposed to be a
     * newline
     */
    @Test
