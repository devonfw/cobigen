...
public class ParsedJavaModelBuilder {
   ...
   *
   * @param javaClass {@link Class} object of the pojo all information should be retrieved from
   ...
   *
   * @param javaClass source {@link JavaClass} to determine all fields accessible via methods from
   ...
   *
   * @param javaClass input java class
   ...
   * @param annotatedElement Annotated Element the field type is source of
   ...
   *         value of meta information or {@code null} if no superclass exist
   */
   ...
   * meta data
   *
   * @param javaClass class for which the setter and getter should be evaluated according to their annotations
  ...
  private void collectAnnotations(JavaClass javaClass, List<Map<String, Object>> attributes) {
    ...
    for (Map<String, Object> attr : attributes) {
      ...
      JavaMethod getter = javaClass.getMethod("get" + StringUtils.capitalize((String) attr.get(ModelConstant.NAME)),
          null, false);
      if (getter != null) {
        extractAnnotationsRecursively(annotations, getter.getAnnotations());
      }

      getter = javaClass.getMethod("is" + StringUtils.capitalize((String) attr.get(ModelConstant.NAME)), null, false);
      if (getter != null) {
        extractAnnotationsRecursively(annotations, getter.getAnnotations());
      }
      ...
      JavaMethod setter = javaClass.getMethod("set" + StringUtils.capitalize((String) attr.get(ModelConstant.NAME)),
          paramList, false);
      if (setter != null) {
  ...
  /**
   * Builds the model for javaDoc. This includes extraction of the comment (without doclets) as well as a mapping of
   * docletTags to its values.
   *
   * @param annotatedElement Annotated element, which javaDoc should be parsed
   * @return the mapping of javaDoc elements to its values or <code>null</code> if the element does not declare javaDoc
   */
   ...
   *
   * @param javaClass {@link Class} object of the POJO the data should be retrieved from
  ...
  private void determinePojoIds(JavaClass javaClass, List<Map<String, Object>> attributes) {
    ...
    for (Map<String, Object> attr : attributes) {
      ...
      try {
        getter = javaClass.getMethod("get" + StringUtil.capFirst((String) attr.get(ModelConstant.NAME)), null, false);
      } catch (Exception e) {
        getter = javaClass.getMethod("is" + StringUtil.capFirst((String) attr.get(ModelConstant.NAME)), null, false);
      }
      if (getter == null) {
        return;
      }
