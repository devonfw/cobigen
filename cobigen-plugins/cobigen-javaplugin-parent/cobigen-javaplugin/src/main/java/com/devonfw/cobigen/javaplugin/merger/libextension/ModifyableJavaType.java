...
public class ModifyableJavaType implements JavaClass, JavaType, Serializable {
  ...
  private JavaClass resolveRealClass() {
    ...
    String qualifiedName = isResolved() ? this.fullName : this.name;
    if (isPrimitive(qualifiedName)) {
      result = new ModifyableJavaClass(qualifiedName);
    } else {
      result = this.typeResolver.getJavaClass(qualifiedName);
    }
    return result;
  }
   ...
   * </pre>
   * <p>
   * We want to know the resolved returnType when calling <code>ConcreteClass.getValue()</code>. The expected type is
   * String.
   * </p>
   *
   * <ul>
  ...
  protected static JavaType resolve(JavaType base, JavaClass declaringClass, JavaClass callingClass) {
      ...
      if (callingClass.getSuperClass() != null && fqn.equals(callingClass.getSuperClass().getFullyQualifiedName())) {
        ...
        for (JavaClass implement : callingClass.getInterfaces()) {
          if (fqn.equals(implement.getFullyQualifiedName())) {
            ...
            JavaType resolvedType = new ModifyableJavaType(actualType.getFullyQualifiedName(), actualType.getValue(),
                getDimensions(base), typeResolver);
    ...
    if (!actualTypeArguments.isEmpty()) {
      ...
      DefaultJavaParameterizedType typeResult = new DefaultJavaParameterizedType(concreteClassName, value,
          getDimensions(base), typeResolver);
  ...
  public String getGenericFullyQualifiedName() {
    ...
    StringBuilder result = new StringBuilder(isResolved() ? this.fullName : this.name);
    for (int i = 0; i < this.dimensions; i++) {
      result.append("[]");
    }
    return result.toString();
  ...
  protected static <D extends JavaGenericDeclaration> String getResolvedGenericFullyQualifiedName(JavaType base,
    ...
    result
        .append(variable == null ? base.getFullyQualifiedName() : variable.getBounds().get(0).getFullyQualifiedName());
    List<JavaType> actualTypeArguments = getActualTypeArguments(base);
    if (!actualTypeArguments.isEmpty()) {
      result.append("<");
      for (Iterator<JavaType> iter = actualTypeArguments.iterator(); iter.hasNext();) {
