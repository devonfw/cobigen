...
public class JavaMergerTest {
	...
	/**
	 * Tests whether the contents will be rewritten after parsing and printing with
	 * QDox with the right encoding
	 * 
	...
	public void testReadingEncoding() throws IOException, MergeException {
		...
		String mergedContents = new JavaMerger("", false).merge(baseFile, FileUtils.readFileToString(patchFile),
				"UTF-8");
	...
	/**
	 * Tests whether all generics of the original file will be existent after
	 * merging
	 * 
	 * @throws IOException    test fails
	 * @throws MergeException test fails
	...
	public void testMergeMethodsWithoutExtendingMethodBodyWithWhitespaces() throws IOException, MergeException {
		...
		String mergedContents = new JavaMerger("", true).merge(file, Files.toString(file, Charset.forName("UTF-8")),
				"UTF-8");
	...
	/**
	 * Tests issue <a href=https://github.com/oasp/cobigen/issues/39>#39</a>:
	 * inheritance relation should be merged also if the base class (natively)
	...
	/**
	 * Tests the behavior if one file imports a type while the other uses an
	 * explicit type. Corresponds to <a
	...
	/**
	 * Calls the {@link JavaMerger} to merge the base and patch file wit the given
	 * overriding behavior
	 * 
	...
	private JavaSource getMergedSource(File baseFile, File patchFile, boolean override)
		...
		String mergedContents = new JavaMerger("", override).merge(baseFile, FileUtils.readFileToString(patchFile),
				"UTF-8");
