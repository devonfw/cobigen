...
public interface InputInterpreter {
   ...
   *
   * @param input object
   * @return <code>true</code> if there is at least one input reader, which interprets the given input as combined
  ...
  /**
   * Reads the content at a path via the first fitting {@link InputReader} and returns a CobiGen compliant input.
   * Supports at least one EXTERNAL input reader with the same isMostLikelyReadable criteria than an INTERNAL input
   * reader.
   *
