...
public interface InputReader {
   ...
   *
   * @param path the {@link Path} to the object. Can also point to a folder
   * @param inputCharset of the input
   * @param additionalArguments depending on the InputReader implementation
   * @return Object that is a valid input
   * @throws InputReaderException if the Path cannot be read
   * @throws IllegalArgumentException if the provided additional arguments do not suffice the used implementation
   */
  public Object read(Path path, Charset inputCharset, Object... additionalArguments) throws InputReaderException;

  /**
   * Try to determine, whether the input file is readable by the plug-ins parser. If false is returned by the method, it
   * is not assured, that the read method is not called at all. In case of the situation, that no plug-in is most likely
   * to read the input, CobiGen will try every input reader to read the input. Please be aware, that this method should
   * be kept highly performant. It does not have to be an exact result.
   *
