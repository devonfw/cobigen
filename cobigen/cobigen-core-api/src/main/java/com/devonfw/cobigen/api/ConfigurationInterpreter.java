...
/**
 * The configuration interpreter is responsible to interpret the underlying CobiGen configuration and e.g. retrieve
 * templates and increment for a specific input.
 */
public interface ConfigurationInterpreter {
   ...
   * @throws InvalidConfigurationException if the configuration of CobiGen is not valid
   */
  @Cached
  ...
  /**
   * Resolves the destination path of a template for a given root path to generate to as well as a given input to be
   * used for variable resolution.
   *
   * @param targetRootPath root path to generate to
   * @param template {@link TemplateTo} to resolve the path for
   * @param input the generator input to be used.
