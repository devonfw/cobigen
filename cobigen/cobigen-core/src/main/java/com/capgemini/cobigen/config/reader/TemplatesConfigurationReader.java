...
public class TemplatesConfigurationReader {
  ...
  public TemplatesConfigurationReader(File file) throws InvalidConfigurationException {
    ...
    try {
      ...
      if (rootNode instanceof TemplatesConfiguration) {
        ...
        throw new InvalidConfigurationException(file, "Unknown Root Node. Use \"templatesConfiguration\" as root Node");
      }
      ...
      Schema schema =
          schemaFactory
              .newSchema(new StreamSource(getClass().getResourceAsStream("/schema/templatesConfiguration.xsd")));
   ...
   * @param triggerInterpreter {@link ITriggerInterpreter} the trigger has been interpreted with
   ...
   * @throws InvalidConfigurationException if there are multiple templates with the same id
  ...
  public Map<String, Template> loadTemplates(Trigger trigger, ITriggerInterpreter triggerInterpreter)
    ...
    for (com.capgemini.Template t : this.configNode.getTemplates().getTemplate()) {
      if (templates.get(t.getId()) != null) {
        throw new InvalidConfigurationException(this.configFile, "Multiple template definitions found for idRef='"
            + t.getId() + "'");
      }
      templates.put(
          t.getId(),
  ...
  private void scanTemplates(TemplateScan scan, Map<String, Template> templates, Trigger trigger,
    ...
    String path = this.configFile.getParent() + "/" + templateFolder;
   ...
   *
   * @param currentDirectory the {@link File} pointing to the current directory to scan.
  ...
  private void scanTemplates(File currentDirectory, String currentPath, TemplateScan scan,
    ...
    for (File child : currentDirectory.listFiles()) {
      if (child.isDirectory()) {
        scanTemplates(child, currentPathWithSlash + child.getName(), scan, templates, trigger, triggerInterpreter);
      } else {
        String templateName = child.getName();
        if (templateName.endsWith(TEMPLATE_EXTENSION)) {
          String templateNameWithoutExtension = templateName.substring(0, templateName.length() - 4);
          ...
          if (!templates.containsKey(templateId)) {
            ...
            Template template =
                new Template(templateId, destinationPath, templateFile, mergeStratgey, outputCharset, trigger,
                    triggerInterpreter);
   ...
   * @param templates {@link Map} of all templates (see
   *        {@link TemplatesConfigurationReader#loadTemplates(Trigger, ITriggerInterpreter)}
   * @param trigger {@link Trigger} for which the templates should be loaded
   * @throws InvalidConfigurationException if there is an invalid idref attribute
  ...
  public Map<String, Increment> loadIncrements(Map<String, Template> templates, Trigger trigger)
    ...
    if (increments != null) {
      for (com.capgemini.Increment source : increments.getIncrement()) {
        generationIncrements.put(source.getId(), new Increment(source.getId(), source.getDescription(), trigger));
      }
      for (com.capgemini.Increment p : this.configNode.getIncrements().getIncrement()) {
   ...
   *
   * @param rootTarget the {@link Increment} on which the templates should be added
   * @param current the source {@link com.capgemini.Increment} from which to retrieve the data
   * @param templates {@link Map} of all templates (see
   *        {@link TemplatesConfigurationReader#loadTemplates(Trigger, ITriggerInterpreter)}
   * @param generationIncrements {@link Map} of all retrieved increments
   * @throws InvalidConfigurationException if there is an invalid idref attribute
  ...
  private void addAllTemplatesRecursively(Increment rootTarget, com.capgemini.Increment current,
    ...
    for (Object ref : current.getTemplateRefOrIncrementRef()) {
      if (ref instanceof TemplateRef) {
        ...
        if (temp == null) {
          throw new InvalidConfigurationException(this.configFile, "No Template found for idRef='" + tRef.getIdref()
              + "'");
        }
   ...
   * @throws InvalidConfigurationException if there is an invalid increment idref
   * @author mbrunnli (11.03.2013)
   */
  private com.capgemini.Increment getIncrementDeclaration(IncrementRef source) throws InvalidConfigurationException {
    ...
    Iterator<com.capgemini.Increment> it =
        this.xPathContext.iterate("increments/increment[@id='" + source.getIdref() + "']");
    ...
    switch (count) {
    case 0:
      throw new InvalidConfigurationException(this.configFile, "No increment definition found for idRef='"
          + source.getIdref() + "'");
    case 1:
      return p;
    default:
      throw new InvalidConfigurationException(this.configFile, "Multiple increment definitions found for idRef='"
          + source.getIdref() + "'");
    }
