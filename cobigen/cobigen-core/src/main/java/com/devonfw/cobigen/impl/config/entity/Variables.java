...
/**
 * This class is a container for variables that can inherit from parent
 * {@link Variables} building a hierarchy. The
 * {@link #containsKey(String) keys} for {@link #get(String) getting} and
 * {@link #put(String, String) setting} variables
 * are internally normalized (see {@link CaseSyntax#normalizeExample(String)})
 * and therefore treated case-insensitive as
 * well as stripped from special characters. Hence, you should name and use
 * variable names in CobiGen templates and
 * their paths accordingly. For legacy support also the original variable name
 * is used with priority so that special
 * characters are still supported for legacy syntax (e.g.
...
public class Variables {
    ...
    /**
     * A {@link Character#isLetter(char) letter} character that is not to be
     * expected to occur in regular input values.
     */
    ...
    /**
     * Regex {@link Pattern} for variable in language-agnostic case-syntax
     * (<code>x_..._x</code>).
     */
     ...
     * The constructor for the root variables.
     *
     * @param properties the internal {@link Properties} with the variables locally
     *                   defined here.
     */
     ...
     *
     * @param parent the parent {@link Variables} to inherit from or {@code null}
     *               for the root {@link Variables}.
     */
     ...
     * @param properties the internal {@link Properties} with the variables locally
     *                   defined here.
     * @param parent     the parent {@link Variables} to inherit from or
     *                   {@code null} for the root {@link Variables}.
     */
     ...
     * @return {@code true} if this {@link Variables} contain the given {@code key},
     ...
     * @return the value of the variable with the given {@code key}. May be
     *         {@code null}.
     */
    ...
    /**
     * @return this {@link Variables} as {@link Map}. Further changes to this
     *         {@link Variables} or one of its ancestors
     *         are NOT reflected by the returned {@link Map}. This is an expensive
     *         operation that may only be called to
     *         pass {@link Variables} to a templating engine that cannot support
    ...
    /**
     * @param folder the {@link Path} pointing to a child-folder potentially
     *               containing
     *               {@link ConfigurationConstants#COBIGEN_PROPERTIES
     *               cobigen.properties}.
     ...
     *         set or this {@link Variables} if no
     ...
     * @return the given {@code string} with all variables (e.g.
     *         <code>${variableName#uncapfirst}</code> or
     *         <code>$_VariableName_$</code>) replaced with those defined by this
     *         {@link Variables}.
     */
     ...
     *
     * @param string            the {@link String} where to resolve all variables.
     * @param replacementForDot the character used as replacement for the dot
     *                          character ('.') or '\0' for no replacement
     ...
     * @param pattern           the {@link Pattern} with the variable syntax. The
     *                          {@link Matcher#group(int) group(1)} has to match
     *                          the actual variable name.
     * @param supportCase       {@code true} for the new case transformation by
     *                          example, {@code false} otherwise.
     * @param replacementForDot the character used as replacement for the dot
     *                          character ('.') or '\0' for no replacement.
     ...
     * @return the resolved {@code value} with the given freemarker function(s)
     *         applied.
     */
    ...
    /**
     * Applies the given {@link String} modifier defined by ?modifier behind the
     * variable reference
     *
