...
/**
 * The {@link CachedInterceptor} enables caching of several requests on the same method. Therefore, the cache relies on
 * the assumption, that the input objects will be hold in memory as long as they are referenced. Due to the fact, that
 * this cache is just utilizing a {@link WeakHashMap}, it will automatically discard entries which are collected by the
 * GC. This class serves as an interceptor for the {@link Cached @Cached} annotation.
 */
public class CachedInterceptor extends AbstractInterceptor {
   ...
   *
   * @param paramHash hash value of the parameters to serve as a key for caching
  ...
  private Object askCache(int paramHash, Method method) {
    ...
    if (this._cache.containsKey(paramHash)) {
      if (this._cache.get(paramHash).containsKey(method)) {
        ...
        return this._cache.get(paramHash).get(method);
      }
    }
    return null;
  }
   ...
   * @param paramHash hash value of the parameters to serve as a key for caching
   * @param method {@link Method} to be called
   * @param returnValue to be cached
   */
  private void persistCache(int paramHash, Method method, Object returnValue) {
    ...
    if (!this._cache.containsKey(paramHash)) {
      ...
      this._cache.put(paramHash, new HashMap<Method, Object>(4));
    }
    this._cache.get(paramHash).put(method, returnValue);
  }
}
