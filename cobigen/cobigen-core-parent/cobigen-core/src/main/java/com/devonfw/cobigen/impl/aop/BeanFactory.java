...
import com.capgemini.cobigen.impl.generator.ConfigurationInterpreterImpl;
import com.capgemini.cobigen.impl.generator.GenerationProcessorImpl;
import com.capgemini.cobigen.impl.generator.InputInterpreterImpl;
import com.capgemini.cobigen.impl.generator.InputResolverImpl;
import com.capgemini.cobigen.impl.generator.MatcherEvaluatorImpl;
import com.capgemini.cobigen.impl.generator.TriggerMatchingEvaluatorImpl;
import com.google.common.collect.Lists;

/**
 * AOP factory with a specific simple scope to not introduce any heavy weight frameworks and make separation
 * of concerns a little bit easier. Will just serve singletons.
 */
public class BeanFactory {
    ...
    /**
     * Creates or returns a new bean matching the interface given as a parameter. The returned instance is
     * always a singleton.
     * @param <T>
     *            super type of the returned instance
     * @param interfaze
     *            to get a bean instance for
    ...
    public <T> T createBean(Class<T> interfaze) {
        ...
        if (registry.containsKey(interfaze.getCanonicalName())) {
            return (T) registry.get(interfaze.getCanonicalName());
        }
    ...
    /**
     * Enforces the creation of a new instance just of the bean for the given interface. Objects for field
     * initialization will be retrieved from the registry. If there is already a bean instance registered for
     * the given interface, this bean will be overwritten by the newly created one. This method should just be
     * used if a new object instance has to be enforced.
     * @param <T>
        ...
        try {
            for (Class<?> c : KNOWN_BEANS) {
                if (interfaze.isAssignableFrom(c)) {
                    ...
                    registry.put(interfaze.getCanonicalName(), ProxyFactory.getProxy(newInstance));
        ...
        for (Field field : newInstance.getClass().getDeclaredFields()) {
            if (field.getAnnotationsByType(Inject.class).length > 0) {
                ...
                try {
                    ...
                    throw new CobiGenRuntimeException("Failure on setting field " + field.getName() + " of class "
                        + newInstance.getClass().getCanonicalName(), e);
                }
                // restore config
    ...
    public void addManuallyInitializedBean(Object bean) {
        registry.put(bean.getClass().getCanonicalName(), bean);
    }
