...
public class TodoInputReader implements InputReader {
    ...
    public List<Object> getInputObjects(Object input, Charset inputCharset, boolean recursively) {
        ...
        List<Object> todoInputObjects = new LinkedList<>();
        ...
        try {
            if (isValidInput(input)) {
                ...
                // Every key found on the root level of the map will be considered an input object
                Set<String> keySet = mapModel.keySet();
                for (String key : keySet) {
                    try {
                        List<Object> inputElements = castToList(mapModel, key);
                        for (Object inputElement : inputElements) {
                            todoInputObjects.add(castToHashMap(inputElement));
                        }
                    } catch (ClassCastException e) {
                        // If it is a primitive type like String, Long or Boolean, we skip it
                        LOG.debug("Key " + key
                            + " has not been succesfully casted to list, most probably it is not a valid input element. Skipping it");
                        continue;
                    }
                }

                return todoInputObjects;
            }
     ...
     */
    private ArrayList<Object> castToList(Map<String, Object> mapModel, String key) throws ClassCastException {
     ...
     */
    private LinkedHashMap<String, Object> castToHashMap(Object o) throws ClassCastException {
