            ...
            for (ObjectProperty propertyPatch : nodesPatch.getPropertyNodes()) {
                ...
                if (propertyBase.getLeft().toSource().equals(propertyPatch.getLeft().toSource())) {
                    ...
                    if (propertyRight instanceof ArrayLiteral) {
                        ...
                        // At the case of non array, and if patchOverrides is true, just replace the entire
                        // property
                        // not doing nothing in case of patchOverrides is false, because the base property is
                        // already added
                    } else {
                        if (patchOverrides) {
